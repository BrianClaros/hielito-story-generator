...
`OPENAI_API_KEY` (and optionally `OPENAI_MODEL`, `OPENAI_IMAGE_MODEL`) is read from a local `.env` (see `.env.example`). Publishing to Instagram additionally needs `IG_ACCESS_TOKEN` and `IG_BUSINESS_ACCOUNT_ID`, plus Cloudflare R2 credentials (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `R2_PUBLIC_BASE_URL`, see below). Using `--image-provider gemini` needs `GEMINI_API_KEY` (and optionally `GEMINI_IMAGE_MODEL`, since Google's model names/availability change — verify against Google AI Studio rather than trusting a hardcoded default long-term).
...
`hielito_daily_story.py` wires each weekday to a 3-tuple `(objetivo_variants, concepto, cost_profile)` (`ESTRATEGIA_POR_DIA` / `obtener_prompt_del_dia`) — `friday`/`saturday` default to `final` quality, the rest to `balanced`; `--cost-profile` on the CLI overrides this per-day default when passed explicitly. `objetivo_variants` is a list of 2-3 differently-worded objectives per weekday (same commercial intent, varied phrasing); `obtener_prompt_del_dia(weekday, seed)` picks one deterministically via `random.Random(f"angle::{weekday}::{seed}")` — same date-seeding pattern as `select_creative_variation` — so the messaging angle rotates week to week instead of repeating forever, while staying reproducible for a given date. It generates the full story image via `generate_complete_openai_story` (same OpenAI Images API path as `hielito_story_generator_V2.py` — no browser automation), then hands the saved PNG to `instagram_publisher.publicar_historia`.

The day's weather also isn't hardcoded: `obtener_clima_del_dia()` calls `weather_client.obtener_clima_actual()`, which queries Open-Meteo's free, keyless current-weather API for a fixed Zona Sur reference point and maps its WMO weather code + temperature onto the same 5-label vocabulary (`hot`/`warm`/`normal`/`cold`/`rainy`) that `choose_template`/`choose_strategy`/`weather_notes` already branch on elsewhere in `hielito_story_generator_V2.py` — so a rainy or hot day now actually changes the template, strategy, and image mood instead of always reading `"normal"`. If the weather API call fails for any reason, it falls back to `"normal"`/no temperature rather than failing the whole run (same philosophy as the OpenAI-copy-generation fallback below). `--weather`/`--temp` on the CLI override the live lookup.
...
Because the Graph API requires a public image URL rather than a file upload, `instagram_publisher.subir_imagen_temporal` uploads the single generated file to a Cloudflare R2 bucket under a random unguessable object key (`R2_BUCKET_NAME`, served publicly via `R2_PUBLIC_BASE_URL` — the bucket's "Public Development URL"), and `borrar_imagen_temporal` deletes the object as soon as Meta has fetched it — see the `finally` block in `publicar_historia`. (This replaced an earlier `pyngrok`/`ThreadingHTTPServer` tunnel approach, which broke in Anthropic's cloud sandbox: the sandbox's mandatory outbound HTTPS proxy makes ngrok's free tier refuse to start — `ERR_NGROK_9009`, "Pay-as-you-go feature" — and is not fixable via network allowlisting. R2 has no such restriction and works the same locally and in the cloud.)

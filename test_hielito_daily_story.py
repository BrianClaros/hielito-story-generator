...
from hielito_story_generator_V2 import CREATIVE_CONCEPTS
...
class ObtenerPromptDelDiaTests(unittest.TestCase):
    def test_covers_every_weekday_with_a_valid_concept(self):
        for weekday in WEEKDAYS:
            objective, concept = obtener_prompt_del_dia(weekday)
            self.assertTrue(objective.strip())
            self.assertIn(concept, CREATIVE_CONCEPTS)

    def test_friday_pushes_asado_concept(self):
        objective, concept = obtener_prompt_del_dia("friday")
